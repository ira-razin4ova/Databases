...
public class QuestService {
    ...
    public QuestFullDto createQuest(CreateQuestDto dto) {
        ...
        if (dto.dateStart() != null || dto.dateEnd() !=null) {
            checkDataStartDataEndQuest(dto);
        }
    ...
    private void checkDataStartDataEndQuest (CreateQuestDto dto) {
        if ((dto.dateStart()== null) != (dto.dateEnd() == null)) {
            throw new ValidationException("укажи ОБЕ даты (старта и окончания), либо не указывай ни одной!");
        }
        if (dto.dateEnd().isBefore(dto.dateStart())) {
            throw new ValidationException("Дата окончания квеста не может быть раньше даты начала! Проверь даты.");
        }
    }
